...
int array[] = {0, 1, 2, 3, 4, 105, 6, 7, 8, 9};
...
void sList_print_test(void) {
    ...
    /* Expected output */
    char buffer[BUFFER_SIZE] = "0, 1, 2, 3, 4, 105, 6, 7, 8, 9";
...
void sList_find_test_suite2(void) {

    sNode_t node = NULL;

    int correct_value = 105;
    int wrong_value = 99;

    CU_ASSERT_EQUAL(sList_find(list, &correct_value, &node), 0);
    CU_ASSERT_NOT_EQUAL(sList_find(list, &wrong_value, &node), 0);
...
int main(int argc, char** argv) {
    ...
    suite2 = CU_add_suite("sList_find", NULL, NULL);
    ...
    /* suite2 */
    if (CU_add_test(suite2, "sList_find_test_suite2", sList_find_test_suite2) == NULL) {
        CU_cleanup_registry();
